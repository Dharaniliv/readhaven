...
import {
  getNonFictionBooksFromDB,
  storeNonFictionBooksInDB,
} from "@/app/lib/db/nonfiction";

export async function GET() {
  try {
    const dbBooks = await getNonFictionBooksFromDB();
    ...
    const booksData = await fetchBooksByCategory("nonfiction", 80);
    ...
    await storeNonFictionBooksInDB(processedBooks);
    ...
    if (process.env.NODE_ENV === "development") {
      console.error("All route Error:", err);
    }
